...
contract AirdropWithFee is ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    IERC20 public airdropToken;
    IERC20 public feeToken; // Token used for fees (can be set to address(0) for ETH fees)
    uint256 public claimFee; // Fee amount required to claim tokens
    ...
    event FeePaid(address indexed beneficiary, address indexed feeToken, uint256 feeAmount);
    ...
    event FeeSettingsUpdated(address indexed feeToken, uint256 claimFee);

    constructor(address _tokenAddress, uint256 _endTime) Ownable(msg.sender) {
    ...
    function setFeeSettings(address _feeToken, uint256 _claimFee) external onlyOwner {
        feeToken = IERC20(_feeToken);
        claimFee = _claimFee;
        emit FeeSettingsUpdated(_feeToken, _claimFee);
    }
    ...
    function claimTokens() external payable nonReentrant {
        ...
        if (address(feeToken) == address(0)) {
            // ETH fee
            require(msg.value >= claimFee, "Airdrop: Insufficient ETH fee");
        } else {
            // ERC20 token fee
            require(claimFee > 0, "Airdrop: Fee amount not set");
            feeToken.safeTransferFrom(msg.sender, address(this), claimFee);
            emit FeePaid(msg.sender, address(feeToken), claimFee);
        }
        ...
        if (address(feeToken) == address(0) && msg.value > claimFee) {
